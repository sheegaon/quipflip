...
> Current implementation notes for the MM workstream.
...
| `captioned` | Player submits a caption after voting | `submit_caption` | Vote must already exist for the round | Current code uses the route gate; a dedicated round-level uniqueness constraint is still a migration task | Caption submission only happens after successful vote | `mm_captions`, `mm_transactions` | `mm_caption_submission` / `mm_caption_free_slot` / `mm_caption_paid` (existing ledger paths) | Caption response returns caption id, cost, free-slot usage, and new wallet balance | Authorship of other captions and pre-vote reveal data stay hidden | Duplicate caption submission is rejected by the route gate; stale caption-round reuse is rejected | Reconnect restores the same round and whether caption submission is still available |
