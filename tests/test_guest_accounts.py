...
class TestGuestAccountCreation:
    ...
    async def test_multiple_guest_accounts_have_unique_emails(self, test_app):
        ...
        response1 = await test_app.post("/player/guest")
        response2 = await test_app.post("/player/guest")
    ...
    async def test_guest_can_login_with_generated_credentials(self, test_app):
        ...
        # Create guest
        create_response = await test_app.post("/player/guest")
...
class TestGuestAccountUpgrade:
    ...
    async def test_upgrade_guest_account(self, test_app):
        ...
        # Upgrade guest
        upgrade_response = await test_app.post(
            "/player/upgrade",
            json={
    ...
    async def test_upgraded_account_preserves_balance(self, test_app, db_session):
        """Test that balance is preserved after upgrade."""
        # Create guest
        create_response = await test_app.post("/player/guest")
        guest_data = create_response.json()
        original_balance = guest_data["balance"]
    ...
    async def test_upgraded_account_cannot_login_with_old_credentials(self, test_app):
        """Test that old guest credentials don't work after upgrade."""
        # Create guest
        create_response = await test_app.post("/player/guest")
        guest_data = create_response.json()
        old_email = guest_data["email"]
        old_password = guest_data["password"]

        # Upgrade guest
        await test_app.post(
            "/player/upgrade",
            json={"email": "upgraded@example.com", "password": "SecurePass123"},
            headers={"Authorization": f"Bearer {guest_data['access_token']}"}
        )
...
class TestGuestRateLimiting:
    """Test guest-specific rate limiting."""

    async def test_guest_creation_rate_limit(self, test_app):
        """Test that guest creation is rate limited per IP."""
        # This test would need to mock IP headers
        # For now, just verify the endpoint accepts IP headers
