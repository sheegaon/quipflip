      ...
      <div className="max-w-5xl w-full tile-card p-6 md:p-10">
        <div className="flex flex-col md:flex-row gap-6 md:items-start">
          ...
          <div className="flex-1 space-y-4">
            <div>
              <p className="text-sm text-quip-teal uppercase tracking-wide">Round complete</p>
              <h1 className="text-3xl font-display font-bold text-quip-navy">Here are your results</h1>
            </div>
            ...
            {voteResult ? (
              <>
                ...
                <div className="p-4 border-2 border-quip-orange rounded-tile bg-white inline-flex gap-2 items-center">
                  <span className="text-quip-navy font-semibold">Earnings</span>
                  <CurrencyDisplay amount={voteResult.payout} />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="border-2 border-quip-navy rounded-tile p-4">
                    <p className="text-sm text-quip-teal">Wallet</p>
                    <CurrencyDisplay amount={voteResult.wallet ?? player?.wallet ?? 0} />
                  </div>
                  <div className="border-2 border-quip-navy rounded-tile p-4">
                    <p className="text-sm text-quip-teal">Vault</p>
                    <CurrencyDisplay amount={voteResult.vault ?? player?.vault ?? 0} />
                  </div>
                </div>
              </>
            ) : (
              <p className="text-quip-teal">No recent round found. Head back to play again!</p>
            )}
