  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-ccl-orange to-ccl-turquoise flex items-center justify-center p-4 bg-pattern">
      <div className="max-w-md w-full tile-card p-8 slide-up-enter space-y-6">
        ...
        {/* Create Party */}
        <div className="bg-ccl-orange bg-opacity-5 border-2 border-ccl-orange rounded-tile p-6">
          <h2 className="text-xl font-display font-bold text-ccl-navy mb-2">Create a Party</h2>
          <p className="text-ccl-teal mb-4 text-sm">
            Host a new party match with 6-9 players. You'll get a code to share with friends.
          </p>
          <button
            onClick={handleCreateParty}
            disabled={isCreating}
            className="w-full bg-ccl-orange hover:bg-ccl-orange-deep disabled:bg-gray-400 text-white font-bold py-3 px-4 rounded-tile transition-all hover:shadow-tile-sm text-lg"
          >
        ...
        <div className="relative">
          <div className="absolute inset-0 flex items-center">
            <div className="w-full border-t border-ccl-teal opacity-30"></div>
          </div>
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-ccl-warm-ivory text-ccl-teal font-semibold">OR</span>
          </div>
        </div>

        {/* Join Party */}
        <div className="bg-ccl-turquoise bg-opacity-5 border-2 border-ccl-turquoise rounded-tile p-6">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-xl font-display font-bold text-ccl-navy">Join a Party</h2>
            <button
              onClick={loadParties}
              disabled={isLoading}
              className="text-ccl-turquoise hover:text-ccl-teal transition-colors"
          ...
          {isLoading && parties.length === 0 ? (
            <div className="text-center py-8 text-ccl-teal">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {parties.map((party) => (
                <div
                  ...
                  className="bg-white border-2 border-ccl-teal rounded-tile p-3 flex items-center justify-between hover:border-ccl-turquoise transition-colors"
                >
                  <div className="flex-1">
                    <p className="font-bold text-ccl-navy">{party.host_username}'s Party</p>
                    <p className="text-sm text-ccl-teal">
                      ...
                      {party.participant_count >= party.min_players && (
                        <span className="ml-2 text-xs bg-ccl-turquoise text-white px-2 py-0.5 rounded">Ready to start</span>
                      )}
                    </p>
                  </div>
                  <button
                    ...
                    className="bg-ccl-turquoise hover:bg-ccl-teal disabled:bg-gray-400 text-white font-bold py-2 px-4 rounded-tile transition-all text-sm"
                  >
