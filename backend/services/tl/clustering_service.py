...
async def calculate_cluster_weight(db: AsyncSession, cluster_id: str) -> float:
...
async def calculate_usefulness(answer: TLAnswer, smoothing: float = 1.0) -> float:
...
async def _update_centroid_raw(db: AsyncSession, cluster_id: str, old_size: int, new_embedding: List[float]) -> None:
...
async def prune_corpus(db: AsyncSession, prompt_id: str, keep_count: int = 1000) -> Tuple[int, int]:
    """Prune inactive answers to maintain corpus cap.

    Strategy:
    1. Mark answers with the lowest usefulness * weight as inactive
...
class TLClusteringService:
    ...
    async def assign_cluster(
        ...
        try:
            ...
            if _is_sqlite(db):
                # SQLite - no type casting needed
                sql_query = """
                    SELECT cluster_id, size, centroid_embedding
                    FROM tl_cluster
            ...
            result = await db.execute(text(sql_query), {"prompt_id": prompt_id})
