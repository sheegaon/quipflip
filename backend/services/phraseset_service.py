...
async def _noop_async_context():
    """Async context manager that performs no transactional work."""
    yield
    ...
    async def claim_prize(
        ...
        try:
            async with transaction_ctx:
                phraseset = await self._lock_phraseset(phraseset_id)
                if not phraseset:
                    raise ValueError("Phraseset not found")
                if phraseset.status != "finalized":
                ...
                if result_view is None:
                    result_view = await self._create_result_view(phraseset, player_id)
