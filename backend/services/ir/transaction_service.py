...
class IRTransactionService:
    ...
    async def _credit_wallet_locked(
        self,
        player_id: str,
        amount: int,
        transaction_type: str,
        ...
        """Credit wallet assuming an external transaction is already open."""
    ...
    async def credit_wallet_in_transaction(
        self,
        player_id: str,
        amount: int,
        transaction_type: str,
        ...
        """Credit a wallet while reusing an existing session transaction."""

        if amount <= 0:
            raise IRTransactionError("amount_must_be_positive")

        if not self.db.in_transaction():
            raise IRTransactionError("transaction_required")

        transaction = await self._credit_wallet_locked(
            player_id=player_id,
            amount=amount,
