    ...
    async def claim_bonus(self, player_id: str) -> dict:
        ...
        try:
            ...
            async with transaction_ctx:
                ...
                transaction = (
                    await self.transaction_service.credit_wallet_in_transaction(
                        player_id=player_id,
                        amount=self.settings.ir_daily_bonus_amount,
                ...
                # Flush both inserts while the transaction is still open so a failure
                # rolls everything back and the caller never sees a half-applied claim.
                await self.db.flush()

            await self.db.refresh(bonus)
            await self.db.refresh(transaction)
        except (IRDailyBonusError, IRTransactionError) as exc:
