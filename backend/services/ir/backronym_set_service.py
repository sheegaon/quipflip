...
from backend.services.ir.word_service import WordService, WordError
from backend.services.ir.queue_service import QueueService

logger = logging.getLogger(__name__)


class BackronymSetError(RuntimeError):
    """Raised when backronym set service fails."""


class BackronymSetService:
    ...
    async def create_set(self, mode: str = Mode.RAPID) -> BackronymSet:
        ...
        try:
            ...
            raise BackronymSetError(f"Failed to generate word: {str(e)}") from e
        except Exception as e:
            await self.db.rollback()
            raise BackronymSetError(f"Failed to create set: {str(e)}") from e

    async def get_set_by_id(self, set_id: str) -> BackronymSet | None:
    ...
    async def get_available_set_for_entry(
        ...
        try:
            ...
            if exclude_player_id:
                existing_entries = select(BackronymEntry.set_id).where(
                    BackronymEntry.player_id == exclude_player_id
                )
                stmt = stmt.where(
                    ~BackronymSet.set_id.in_(existing_entries)
                )

            # Order by created_at DESC (most recent first)
            stmt = stmt.order_by(BackronymSet.created_at.desc()).limit(1)
    ...
    async def add_entry(
        ...
        try:
            ...
            if set_obj.status != SetStatus.OPEN:
                raise BackronymSetError("set_not_open")
            ...
            if not is_ai:
                ...
                if set_obj.first_participant_joined_at is None:
                    ...
                    observer_guard = BackronymObserverGuard(
                        set_id=set_id,
        ...
        except BackronymSetError:
            raise
        except Exception as e:
            await self.db.rollback()
            raise BackronymSetError(f"Failed to add entry: {str(e)}") from e

    async def transition_to_voting(self, set_id: str) -> BackronymSet:
        ...
        try:
            ...
            if set_obj.status != SetStatus.OPEN:
                raise BackronymSetError("set_not_in_open_status")

            set_obj.status = SetStatus.VOTING
            ...
            await self.db.rollback()
            raise BackronymSetError(f"Failed to transition to voting: {str(e)}") from e
    ...
    async def add_vote(
        ...
        try:
            ...
            if set_obj.status != SetStatus.VOTING:
                raise BackronymSetError("set_not_in_voting_phase")
            ...
            entry = (
                await self.db.execute(
                    select(BackronymEntry).where(
                        BackronymEntry.entry_id == chosen_entry_id
                    )
                )
            ).scalars().first()
            if entry:
            ...
            participant_votes = (
                await self.db.execute(
                    select(BackronymVote).where(
                        and_(
                            BackronymVote.set_id == set_id,
                            BackronymVote.is_participant_voter == True,
                        )
                    )
                )
            ).scalars().all()
            ...
            await self.db.rollback()
            raise BackronymSetError(f"Failed to add vote: {str(e)}") from e

    async def finalize_set(self, set_id: str) -> BackronymSet:
        ...
        try:
            ...
            await self.db.rollback()
            raise BackronymSetError(f"Failed to finalize set: {str(e)}") from e
    ...
    async def get_set_details(self, set_id: str) -> dict:
        ...
        try:
            ...
            # Get votes
            votes_stmt = select(BackronymVote).where(
                BackronymVote.set_id == set_id
            )
            ...
            raise BackronymSetError(f"Failed to get set details: {str(e)}") from e

    async def get_stalled_open_sets(self, minutes: int = 2) -> list[BackronymSet]:
        ...
        Returns:
            list[BackronymSet]: Sets needing AI entries
        """
        try:
            ...
            stmt = select(BackronymSet).where(
                and_(
                    BackronymSet.status == SetStatus.OPEN,
                    BackronymSet.entry_count < 5,
                    BackronymSet.created_at <= cutoff_time,
                )
            )
            result = await self.db.execute(stmt)
    ...
    async def get_stalled_voting_sets(self, minutes: int = 2) -> list[BackronymSet]:
        ...
        Returns:
            list[BackronymSet]: Sets needing AI votes
        """
        try:
            cutoff_time = datetime.now(UTC) - timedelta(minutes=minutes)
            stmt = select(BackronymSet).where(
                and_(
                    BackronymSet.status == SetStatus.VOTING,
                    BackronymSet.vote_count < 5,
                    # Check when first participant joined or created
                    or_(
                        BackronymSet.first_participant_joined_at <= cutoff_time,
                        and_(
                            BackronymSet.first_participant_joined_at == None,
                            BackronymSet.created_at <= cutoff_time,
                        ),
                    ),
                )
