...
class IRStatisticsService:
    ...
    async def get_player_stats(self, player_id: str) -> dict:
        ...
        try:
            ...
            # Count entries
            entries_stmt = select(func.count(IRBackronymEntry.entry_id)).where(
                (IRBackronymEntry.player_id == player_id)
                & (IRBackronymEntry.is_ai == False)
            )
            entries_result = await self.db.execute(entries_stmt)
            entry_count = entries_result.scalar() or 0

            # Count votes
            votes_stmt = select(func.count(IRBackronymVote.vote_id)).where(
                (IRBackronymVote.player_id == player_id)
                & (IRBackronymVote.is_ai == False)
            )
    ...
    async def get_creator_leaderboard(self, limit: int = 10) -> list[dict]:
        ...
        try:
            ...
            for rank, player in enumerate(players, 1):
                ...
                entries_stmt = select(func.count(IRBackronymEntry.entry_id)).where(
                    (IRBackronymEntry.player_id == str(player.player_id))
                    & (IRBackronymEntry.is_ai == False)
                )
                entries_result = await self.db.execute(entries_stmt)
                entry_count = entries_result.scalar() or 0
    ...
    async def get_voter_leaderboard(self, limit: int = 10) -> list[dict]:
        ...
        try:
            ...
            voter_stmt = (
                select(
                    IRBackronymVote.player_id,
                    func.count(IRBackronymVote.vote_id).label("vote_count"),
                )
                .where(
                    (IRBackronymVote.is_ai == False)
                    & (IRBackronymVote.is_participant_voter == False)
                )
                .group_by(IRBackronymVote.player_id)
                .order_by(func.count(IRBackronymVote.vote_id).desc())
                .limit(limit)
            )
    ...
    async def get_set_statistics(self, set_id: str) -> dict:
        ...
        try:
            ...
            # Count human vs AI
            human_entries_stmt = select(func.count(IRBackronymEntry.entry_id)).where(
                (IRBackronymEntry.set_id == set_id)
                & (IRBackronymEntry.is_ai == False)
            )
            ...
            human_votes_stmt = select(func.count(IRBackronymVote.vote_id)).where(
                (IRBackronymVote.set_id == set_id)
                & (IRBackronymVote.is_ai == False)
            )
