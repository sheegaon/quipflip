...
def upgrade() -> None:
    ...
    # Get the current dialect for proper foreign key handling
    bind = op.get_bind()
    ...
    sa.Column('wordset_id', uuid, nullable=True),
    ...
    sa.Column('wordset_id', uuid, nullable=False),
    sa.Column('prompt_round_id', uuid, nullable=False),
    sa.Column('copy_round_1_id', uuid, nullable=False),
    sa.Column('copy_round_2_id', uuid, nullable=False),
    ...
    sa.Column('view_id', uuid, nullable=False),
    sa.Column('wordset_id', uuid, nullable=False),
    sa.Column('player_id', uuid, nullable=False),
    ...
    sa.Column('vote_id', uuid, nullable=False),
    sa.Column('wordset_id', uuid, nullable=False),
    sa.Column('player_id', uuid, nullable=False),
...
def downgrade() -> None:
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    # Handle cross-table foreign key removal with dialect-specific approach
    if bind.dialect.name == 'postgresql':
        ...
        # SQLite: Use batch mode to drop foreign keys
        with op.batch_alter_table('rounds', schema=None) as batch_op:
            batch_op.drop_constraint('fk_rounds_wordset_id', type_='foreignkey')
        
        with op.batch_alter_table('players', schema=None) as batch_op:
            batch_op.drop_constraint('fk_players_active_round_id', type_='foreignkey')
